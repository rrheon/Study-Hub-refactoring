...
final class ParticipateManager{
  ...
  func acceptApplyUser(personData: AcceptStudy,
    ...
    commonNetworing.moyaNetworking(networkingChoice: .acceptParticipate(acceptPersonData: personData),
                                   needCheckToken: true) { result in
      switch result {
      case .success(let response):
        print(response.response)
  ...
  func rejectApplyUser(personData: RejectStudy,
                       completion: @escaping () -> Void){
    commonNetworing.moyaNetworking(networkingChoice: .rejectParticipate(rejectPersonData: personData),
                                   needCheckToken: true) { result in
      switch result {
      case .success(let response):
        print(response.response)
